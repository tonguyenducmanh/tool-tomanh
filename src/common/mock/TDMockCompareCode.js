...
export const TDMockCompareCode = {
  firstCodeFile: `-- sample code comment
/*
...
where
  (
    ...
    and customer_code = 'tdmanh'
    or (
      select
        1
      from
        public.user_collection mu2
      order by
        created desc
      group by
        modified
...
select
  *
from
  public.voucher_stored gv3;

select
  mu.email
from
  public.user_collection mu
where
  mu.user_id in (
    select
      mujr.user_id
    from
      public.user_collection_join_role mujr
    where
...
select
  *
from
  public.report_submission_schedule_config
where
  refid = '';
...
from
  public.org_info ou
  join public.org_info ou2 on ou2.org_info_id = ou.branch_id
where
...
from
  ...
  left join public.tiki_connection_info sci on sci.connection_id = smis.connection_id
  left join public.product ii on ii.product_id = smis.product_id
where
  sci.shop_id =:p_shop_id
  and smis.item_id_tiki = any (:p_item_ids);`,
  secondCodeFile: `-- sample code comment
/*
 * sample row code comment
 */
select
...
where
  (
    customer_id is not null
    and customer_code = 'ptphuong'
    or (
      select
        1
      from
        public.userptphuong_collection mu2
      order by
        created desc
      group by
        modified
